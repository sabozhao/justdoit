  ...
  <div class="library">
    ...
    <div class="breadcrumb-container">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <span @click="goToRoot" class="breadcrumb-link">共享题库</span>
        </el-breadcrumb-item>
    ...
    <!-- 操作栏（仅管理员可见） -->
    <div class="library-toolbar" v-if="authStore.user && authStore.user.is_admin">
    ...
    <!-- 搜索栏（普通用户可见） -->
    <div class="library-toolbar" v-else>
    ...
    <div class="library-content">
      ...
      >
        <el-table-column v-if="authStore.user && authStore.user.is_admin" type="selection" width="55" />
        ...
        <el-table-column label="操作" width="300" fixed="right">
          <template #default="{ row }">
            <div class="action-buttons">
              ...
              <el-button
                v-if="authStore.user && authStore.user.is_admin && row.type === 'category'"
                type="primary"
                link
                size="small"
              ...
              <el-button
                v-if="authStore.user && authStore.user.is_admin && row.type === 'bank'"
                type="primary"
                link
                size="small"
      ...
      <div v-if="!loading && tableData.length === 0" class="empty-state">
        <el-empty description="暂无数据">
          <el-button v-if="authStore.user && authStore.user.is_admin" type="primary" @click="openUploadDialog">
            上传第一个题库
          </el-button>
        </el-empty>
      </div>
...
解析：这是多选题的解析</pre>
    ...
    <el-dialog 
      ...
      :title="currentBank ? `${currentBank.name} - 题目管理` : '题目管理'" 
      width="900px"
    >
    ...
    <el-dialog 
      ...
      :title="currentQuestion.id ? '编辑题目' : '添加题目'" 
      width="600px"
    >
...
import { API_BASE_URL, publicCategoryAPI, publicQuestionBankAPI } from '@/api'
...
export default {
  ...
  setup() {
    ...
    const loadCategories = async () => {
      try {
        const result = await publicCategoryAPI.getAll()
      ...
      if (searchKeyword.value && searchKeyword.value.trim()) {
        const filterByKeyword = (items) => {
          ...
          for (const item of items) {
            ...
            const filteredChildren = item.children && item.children.length > 0 ? filterByKeyword(item.children) : []
    ...
    const saveCategory = async () => {
      ...
      try {
        if (isEditingCategory.value) {
          await publicCategoryAPI.update(categoryForm.value.id, {
          ...
          await publicCategoryAPI.create({
            name: categoryForm.value.name,
            description: categoryForm.value.description,
            parent_id: categoryForm.value.parent_id,
            sort_order: categoryForm.value.sort_order
    ...
    const deleteCategory = async (category) => {
      try {
        ...
        await publicCategoryAPI.delete(category.id)
    ...
    const loadBanks = async () => {
      try {
        ...
        const banks = await publicQuestionBankAPI.getAll(params)
        examStore.questionBanks = banks || []
      } catch (error) {
        console.error('加载共享题库失败:', error)
        ElMessage.error('加载共享题库失败: ' + error.message)
        examStore.questionBanks = []
      }
    }
    ...
    const uploadQuestionBank = async () => {
      ...
      try {
        ...
        formData.append('parseMode', uploadForm.parseMode)
        formData.append('bankName', uploadForm.name)
        ...
        const result = await publicQuestionBankAPI.uploadFile('new', formData)
    ...
    const createQuestionBank = async () => {
      ...
      try {
        ...
        await publicQuestionBankAPI.create({
          name: createForm.name,
          description: createForm.description,
          category_id: createForm.categoryId || null,
          questions: questions
        })
    ...
    const saveEditBank = async () => {
      ...
      try {
        ...
        await publicQuestionBankAPI.update(editForm.id, updateData)
        await loadBanks()
    ...
    const handleDelete = async (row, skipConfirm = false) => {
      ...
      if (row.type === 'category') {
        ...
        try {
          ...
          await publicQuestionBankAPI.delete(bankId)
          if (!skipConfirm) {
    ...
    const manageQuestions = async (rowOrId) => {
      ...
      try {
        ...
        if (!bank || !bank.id) {
          try {
            const bankDetails = await publicQuestionBankAPI.getById(bankId)
            currentBank.value = bankDetails
          } catch (err) {
            ElMessage.error('共享题库不存在或数据不完整')
            return
          }
        } else {
        ...
        const questions = await publicQuestionBankAPI.getQuestions(bankId)
    ...
    const saveQuestion = async () => {
      ...
      try {
        ...
        if (currentQuestion.value.id) {
          await publicQuestionBankAPI.updateQuestion(currentQuestion.value.id, questionData)
          ElMessage.success('题目更新成功')
        } else {
          await publicQuestionBankAPI.addQuestion(questionData)
    ...
    const deleteQuestion = async (question) => {
      ...
      try {
        ...
        await publicQuestionBankAPI.deleteQuestion(question.id)
